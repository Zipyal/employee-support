        ...
        <form method="post" action="{{ $formUrl }}" class="row g-3 needs-validation">
                ...
                <div class="input-group has-validation">
                    <select id="type" name="employee_uuid"
                        ...
                        @foreach ($employees as $i => $employee)
                            ...
                            @endif
                                <option value="{{ $employee->uuid }}" {{ (old('employee_uuid') ?? $task->employee_uuid) == $employee->uuid ? 'selected' : '' }}>{{ $employee->fullName }}</option>
            ...
            <div class="col-12">
                <div class="row bg-light py-3">
                    ...
                    <div class="col-12 col-md-4">
                        <label for="material_uuid" class="form-label">Прикрепить материал</label>
                        <div class="input-group has-validation">
                            <select id="type" name="material_uuid" class="form-select @error('material_uuid') is-invalid @enderror">
                                <option value=""> - Не выбрано -</option>
                                @foreach ($materials as $uuid => $subject)
                                    <option value="{{ $uuid }}" {{ (old('material_uuid') ?? $task->material_uuid) == $uuid ? 'selected' : '' }}>{{ $subject }}</option>
                                @endforeach
                            </select>
                            @error('material_uuid')
                            <div class="invalid-feedback">{{ $message }}</div>
